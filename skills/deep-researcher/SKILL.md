...
description: Produce evidence-led, multi-source research reports with persistent claim tracking, retrieval coverage, and contradiction analysis. Use for substantive investigations and comparisons; simple lookups, single-claim checks, and synthesis of an already supplied corpus usually need a smaller workflow.
license: MIT
metadata:
  hermes:
...
Use the host's native search, extraction, browser, and file tools. Do not type tool calls
...
For a configured Hermes research-access installation, also load the companion
`hermes-research-access` skill. The general workflow does not assume that plugin is installed.
Native retrieval tools own extraction; do not bypass them with terminal Firecrawl calls,
inline Python, or generated extraction scripts. Invoke the bundled bookkeeping scripts.

## Record evidence and checkpoint
